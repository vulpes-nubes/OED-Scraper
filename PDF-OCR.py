...
class PDFOCRApp:
    def __init__(self, root):
        self.root = root
        self.root.title("PDF OCR Tool (macOS)")
    ...
    def create_widgets(self):
        ...
        lang_menu = ttk.Combobox(frame, textvariable=self.lang, values=['eng', 'deu', 'fra', 'spa', 'ita'], state="readonly")
    ...
    def process_pdfs(self):
        ...
        for i, pdf_path in enumerate(self.pdf_paths, start=1):
            print(f"[INFO] Processing ({i}/{total}): {os.path.basename(pdf_path)}")
            self.convert_pdf_to_searchable(pdf_path)
            self.progress["value"] += 1

        messagebox.showinfo("Done", "OCR process completed!")
